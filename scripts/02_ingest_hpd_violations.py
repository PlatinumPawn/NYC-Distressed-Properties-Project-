#!/usr/bin/env python3
"""
HPD Violations Data Ingestion Pipeline
Fetches NYC HPD Violations data from SODA API with pagination and rate limiting
...
OUTPUT_FILE = OUTPUT_DIR / 'hpd_violations.parquet'
CHECKPOINT_FILE = OUTPUT_DIR / 'hpd_checkpoint.json'
TEMP_FILE = OUTPUT_DIR / 'hpd_violations_temp.parquet'
...
def save_checkpoint(offset, batch_num, total_fetched, date_col):
...
def detect_date_column(client, dataset_id, start_date):
    """Detect which date column to use for filtering"""
    date_columns_to_try = ['novissueddate', 'approveddate', 'inspectiondate']
    ...
    for date_col in date_columns_to_try:
        try:
            ...
            result = client.get(
                ...
                where=f"{date_col} >= '{start_date}T00:00:00.000'",
                limit=1
            )
            total = int(result[0]['count'])
            print(f"  ✓ Using '{date_col}' - found {total:,} records")
            return date_col, total
        except Exception as e:
            print(f"  ✗ '{date_col}' failed: {e}")
            continue

    raise ValueError("Could not find valid date column for HPD dataset")

def validate_batch(batch_df, batch_num, date_col):
...
def fetch_hpd_data(app_token=None, resume=False):
    """
    Fetch HPD Violations with incremental saving
    ...
    """
    print("="*80)
    print("HPD VIOLATIONS DATA INGESTION")
    print("="*80)
    print(f"\nConfiguration:")
    ...
    if checkpoint:
        ...
        print(f"  Date column: {checkpoint['date_col']}")
    ...
    try:
        # Detect date column if not resuming
        if date_col is None:
            print("Detecting date column...")
            date_col, total_records = detect_date_column(client, DATASET_ID, START_DATE)
        else:
            # Get count with existing date column
            try:
                result = client.get(
                    DATASET_ID,
                    select="COUNT(*) as count",
                ...
                total_records = int(result[0]['count'])
                print(f"  Using date column: {date_col} ({total_records:,} records)")
            except Exception:
                total_records = None
        ...
        while True:
            ...
            while retry_count < MAX_RETRIES and not batch_success:
                try:
                    ...
                    validate_batch(batch_df, batch_num, date_col)

                    # Convert date columns
                    date_cols_to_convert = [date_col, 'novissueddate', 'approveddate',
                                           'currentstatusdate', 'originalcorrectbydate', 'inspectiondate']
                    ...
                    # Save checkpoint
                    save_checkpoint(offset + BATCH_SIZE, batch_num + 1, total_fetched, date_col)
        ...
        return total_fetched, date_col

    finally:
        client.close()

def deduplicate_and_finalize(date_col):
    ...
    # Deduplicate on violationid if available, otherwise on combination of fields
    dedup_cols = []
    if 'violationid' in df.columns:
        dedup_cols = ['violationid']
    elif 'inspectionid' in df.columns and date_col in df.columns:
        dedup_cols = ['inspectionid', date_col]
    ...
    try:
        metadata = {
            ...
            'date_range': f"{df[date_col].min()} to {df[date_col].max()}" if date_col in df.columns else 'N/A',
            'ingestion_timestamp': datetime.now().isoformat(),
            'date_column_used': date_col,
            'boroughs': df['boroid'].nunique() if 'boroid' in df.columns else 'N/A',
            'classes': df['class'].nunique() if 'class' in df.columns else 'N/A',
            'valid_bbl_pct': f"{bbl_pct:.1f}%" if 'bbl' in df.columns else 'N/A'
        }

        metadata_file = OUTPUT_FILE.parent / 'hpd_metadata.txt'
...
def main():
    ...
    try:
        # Fetch data
        result = fetch_hpd_data(app_token, resume=resume)

        if result and result[0] > 0:
            total_fetched, date_col = result

            # Deduplicate and finalize
            df = deduplicate_and_finalize(date_col)
            ...
            if df is not None:
                ...
                if 'boroid' in df.columns:
                    print(f"\nViolations by Borough ID:")
                    print(df['boroid'].value_counts().sort_index().to_string())
